...
Prune entries by editing `config.json` directly. Adding a source is the same operation in reverse — append a new entry with a unique `id`.
...
- The source's `tier` from `config.json` is a prior, not a guarantee. A `tier: high` source can publish low-relevance items; a `tier: low` source can occasionally publish gold. Score the *item*, not the *source*.
